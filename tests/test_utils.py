...
def test_data_for_transactions_valid_json_list():
    ...
    with patch("builtins.open", mock_open(read_data=mock_file_content)), patch("json.load") as mock_json_load:

        mock_json_load.return_value = [
            {
                "id": 441945886,
                "state": "EXECUTED",
                "operationAmount": {"amount": "31957.58", "currency": {"code": "EUR"}},
            }
        ]
...
def test_data_for_transactions_not_a_list():
    with patch("builtins.open", mock_open(read_data='{"not": "a list"}')), patch(
        "json.load", return_value={"not": "a list"}
    ):
